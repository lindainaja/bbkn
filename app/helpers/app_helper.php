...
function array_kv($arr, $key_field, $value_field)
{
    $tmp=[];
    foreach ($arr as $r) {
        $kkey= $r[$key_field];
        unset($r[$key_field]);
        $tmp[$kkey]=$r[$value_field];
    }

    return $tmp;
}

function array_ids($arr, $pk)
{
    $tmp=[];
    foreach ($arr as $r) {
        $kkey= $r[$pk];
...
function strip_rpmask($str){
    return str_replace(['Rp ','.',','], ['','','.'], $str);;
}

function extract_date_range_id($str){
    //08/11/2018_08/24/2018
    $data = explode('_', $str);
    return [
        'dt_min' => date_format_mysql($data[0]),
        'dt_max' => date_format_mysql($data[1])
    ];
}

function extract_mysql_date_info($date){
    $date = explode('-', $date);
    $data = [
        'y' => $date[0],
        'm' => $date[1],
...
function extract_f($suffix){
    $key = 'F-' . preg_replace('/^(\d?)(\d+)/','$1.$2',$suffix);
    if(preg_match('/_/',$suffix)){
        $suf_split = explode('_', $suffix);
        $key = 'F-' . preg_replace('/^(\d?)(\d+)/','$1.$2',$suf_split[0]).'-'.preg_replace('/^(\d?)(\d+)/','$1.$2',$suf_split[1]);
    }
    return $key;
}