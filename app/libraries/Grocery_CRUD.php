...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
    ...
    public function get_view_file_base()
    {
        if (empty($this->_view_file_base)) {
            return $this->theme_path.$this->theme.'/views/';
        }
        return $this->_view_file_base;
    }
    public function set_view_file($key, $value)
    {
        $this->_view_files[$key] = $value;
    }
    public function get_view_file($key)
    {

        // die( $this->theme_path.$this->theme.'/views/'.$key.'.php');
        $key = str_replace('.php', '', $key);
        if (!empty($this->_view_files[$key])) {
            return $this->_view_files[$key];
        } else {
            $override =  $this->get_view_file_base().'/'.$key.'.php';
            if (file_exists($override)) {
                return $override;
            }
            return $this->theme_path.$this->theme.'/views/'.$key.'.php';
        }
        return $key;
    }
    protected function set_basic_Layout()
    {
        if (!file_exists($this->get_view_file('list_template'))) {
    ...
    protected function showList($ajax = false, $state_info = null)
    {
        ...
        $data->export_url              = $this->getExportUrl();
        $data->upload_button_url              = $this->getUploadButtonUrl();
        $data->download_url              = $this->getDownloadUrl();
        $data->upload_url             = $this->getUploadButtonUrl();
        ...
        foreach ($data->list as $num_row => $row) {
            ...
            $data->list[$num_row]->download_url = $data->download_url.'/'.$row->{$data->primary_key};
            $data->list[$num_row]->upload_url = $data->upload_url.'/'.$row->{$data->primary_key};
            $data->list[$num_row]->upload_url = $data->upload_url.'/'.$row->{$data->primary_key};
        }
        ...
        if (!$ajax) {
            ...
            $data->list_view = $this->_theme_view('list.php', $data, true);
            $this->_theme_view('list_template', $data);
        } else {
            $this->set_echo_and_die();
            $this->_theme_view('list', $data);
        }
    }
    ...
    protected function get_success_message_at_list($field_info = null)
    {
        if ($field_info !== null && isset($field_info->success_message) && $field_info->success_message) {
            if (!empty($field_info->primary_key) && !$this->unset_edit) {
                return $this->subject . ' ' . $this->l('insert_success_message')." <a class='go-to-edit-form' href='".$this->getEditUrl($field_info->primary_key)."'>".$this->l('form_edit')." {$this->subject}</a> ";
            } else {
    ...
    protected function get_integer_input($field_info, $value)
    {
        ...
        $input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$value' class='numeric form-control form-control' $extra_attributes />";
        return $input;
    }
    ...
    protected function get_true_false_input($field_info, $value)
    {
        ...
        $input .= "<label><input id='field-{$field_info->name}-true' class='radio-uniform form-control'  type='radio' name='{$field_info->name}' value='1' $checked /> ".$true_string."</label> ";
        ...
        $input .= "<label><input id='field-{$field_info->name}-false' class='radio-uniform form-control' type='radio' name='{$field_info->name}' value='0' $checked /> ".$false_string."</label>";
    ...
    protected function get_password_input($field_info, $value)
    {
        ...
        $input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='password' value='$value' $extra_attributes class=' form-control'/>";
        return $input;
    }
    ...
    protected function get_date_input($field_info, $value)
    {
        ...
        $input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$date' maxlength='10' class='datepicker-input form-control' data-plugin='datepicker'/>
        <a class='datepicker-input-clear' tabindex='-1'>"."</a> (".$this->ui_date_format.")";
        return $input;
    }
    ...
    protected function get_upload_file_input($field_info, $value)
    {
        ...
        $input .= "<a target='_blank' href='".$file_url."' id='file_$unique' class='open-file";
        $input .= $is_image ? " $image_class'><img src='".$file_url."' height='50px'>" : "' target='_blank'>$value";
        $input .= "</a> ";
        $input .= "<a href='javascript:void(0)' id='delete_$unique' class='delete-anchor'>".'<i class="icon wb-trash"></i>'."</a> ";
    ...
    protected function _theme_view($view, $vars = array(), $return = false)
    {
        ...
        $view_file = $this->get_view_file($view);
        ...
        if (! $file_exists) {
            throw new Exception('Unable to load the requested file: '.$view_file, 16);
        }
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
    ...
    protected function getDownloadUrl($primary_key = null)
    {
        if ($primary_key === null) {
            return $this->state_url('download');
        } else {
            return $this->state_url('download/'.$primary_key);
        }
    }
    protected function getExportUrl($primary_key = null)
    {
        if ($primary_key === null) {
            return $this->state_url('export');
        } else {
            return $this->state_url('export/'.$primary_key);
        }
    }
    protected function getUploadButtonUrl($primary_key = null)
    {
        if ($primary_key === null) {
            return $this->state_url('upload');
        } else {
            return $this->state_url('upload/'.$primary_key);
        }
    }
...
class Grocery_CRUD extends grocery_CRUD_States
{
    ...
    protected $default_language_path                = 'public/assets/gc/languages';
    protected $default_config_path                  = 'public/assets/gc/config';
    protected $default_assets_path                  = 'public/assets/gc';
...
class UploadHandler
{
    ...
    private function trim_file_name($name, $type)
    {
        ...
        return   preg_replace("/\s*|\W(?=.*\.[^.]*$)/", "", $file_name);
    }
    ...
    private function _transliterate_characters($file_name)
    {
        ...
        $filepath = str_replace(" ", "-", $transformed_file_name);
        $filepath = str_replace("(", "-", $transformed_file_name);
        $filepath = str_replace(")", "-", $transformed_file_name);
        return $filepath;
    }
