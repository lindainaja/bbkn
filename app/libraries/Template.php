...
class Template {
    ...
    *
    * @access  public
    * @param   string   Nama to identify the region
      ...
      if (isset($this->regions[$region]))
      {
         if ($overwrite === TRUE) // Should we append the content or overwrite it
         {
          ...
          if( !is_array($this->regions[$region])){
            $this->regions[$region] = array('content'=>array());
          }
   ...
   function write_view($region, $view, $data = NULL, $overwrite = FALSE)
   {
      ...
      // echo $view;
      // print_r(get_class($this->CI->load));
   ...
   function _build_content($region, $wrapper = NULL, $attributes = NULL)
   {
	...
	{
		$title = '<div class="page-title"><div><h1>'.$title.'</h1></div></div>';
		return $this->write('_title_page', $title);
	}

	function add_breadcrumb($data=array())
	{
		$breadcrumb = NULL;
		...
		$this->CI->load->helper('url');
                    ...
                    /*<ol class="breadcrumb page-breadcrumb hidden-xs">
                        <li><i class="fa fa-home"></i>&nbsp;<a href="index-2.html">Beranda</a>&nbsp;&nbsp;<i
		...
		$separator = '<span class="divider"><i class="fa fa-angle-right"></i></span>';
		$url = '';
		$tpl = '<div id="breadcrumbs"><ul class="breadcrumb">';
		$i=0;
		foreach($data as $key => $val){
			...
			if($i==count($data)-1){
				...
				$url = '<a href="'.site_url($key).'">'.$val.'</a>';
				$class = '';
			}
			$tpl .= '<li '.$class.'>'.$icon.' '.$url.''.$separator.'</li>';
			$i++;
		}
		$tpl .= '</ul></div>';
		$this->breadcrumb[] = $tpl;
		return $this->write('_breadcrumb', $tpl);;
   }
