...
class CMS_Asset
{
    ...
    {
        if (isset($layout)) {
            $this->add_js(base_url('app/themes/' . $theme . '/assets/' . $layout . '/' . $path));
        } else {
            $this->add_js(base_url('app/themes/' . $theme . '/assets/' . $path));
        }
    }
    ...
    {
        if (isset($layout)) {
            $this->add_css(base_url('app/themes/' . $theme . '/assets/' . $layout . '/' . $path));
        } else {
            $this->add_css(base_url('app/themes/' . $theme . '/assets/' . $path));
        }
    }

    public function add_module_js($path, $module)
    {
        $this->add_js(base_url('app/modules/' . $module . '/assets/' . $path));
    }

    public function add_module_css($path, $module)
    {
        $this->add_css(base_url('app/modules/' . $module . '/assets/' . $path));
    }
    ...
    private function combine_css($resources, $extension = 'css')
    {
        ...
        $file_name = BASEPATH . '../publlic/assets/caches/' . $md5_name . '.' . $extension;
        $file_url  = base_url('public/assets/caches/' . $md5_name . '.' . $extension);
