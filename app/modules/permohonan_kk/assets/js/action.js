...
function upload_data(permohonan_kkid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {permohonan_kkid:permohonan_kkid},
		cache : false,
		dataType : "html",
...
function edit_data(permohonan_kkid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form() && validasi_anggota()){
		var datas = $('#form_permohonan_kk').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var permohonan_kkid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+permohonan_kkid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(permohonan_kkid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(permohonan_kkid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(permohonan_kkid) {
	$.ajax({
		type : "POST",
