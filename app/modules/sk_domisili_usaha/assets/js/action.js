...
function upload_data(sk_domisili_usahaid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_domisili_usahaid:sk_domisili_usahaid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_domisili_usahaid) {
	$.ajax({
		type : "POST",
...
function validasi_form(){
	...
	$(".wajib_isi").each(function (e) {
		var id = $(this).attr('id');
		var val = $("#"+id).val();
		var caption = $("#"+id).closest('.form-group').find('label').text().replace(/\n/,'');
		if(val==''){
			status = false;
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_domisili_usaha').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_domisili_usahaid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_domisili_usahaid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_domisili_usahaid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_domisili_usahaid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_domisili_usahaid) {
	$.ajax({
		type : "POST",
