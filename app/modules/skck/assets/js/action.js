...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_skck').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var skckid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+skckid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(skckid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(skckid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(skckid) {
	$.ajax({
		type : "POST",
