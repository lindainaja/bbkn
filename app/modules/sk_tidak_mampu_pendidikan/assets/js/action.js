...
function upload_data(sk_tidak_mampu_pendidikanid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_tidak_mampu_pendidikanid:sk_tidak_mampu_pendidikanid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_tidak_mampu_pendidikanid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_tidak_mampu_pendidikan').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_tidak_mampu_pendidikanid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_tidak_mampu_pendidikanid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_tidak_mampu_pendidikanid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_tidak_mampu_pendidikanid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_tidak_mampu_pendidikanid) {
	$.ajax({
		type : "POST",
