...
class M_sk_tidak_mampu_pendidikan extends CI_Model 
{
	...
	{
		return $this->db->where('no_kec',$this->no_kec)->get('setup_kec')->row_array();
	}
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_sk_tidak_mampu_pendidikan pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_sk_tidak_mampu_pendidikan',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_sk_tidak_mampu_pendidikan',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_sk_tidak_mampu_pendidikan');
	}



	function detail_data_anggota($pk=0)
	{
		$this->db->select('pkd.*',false);
		$this->db->from('t_sk_tidak_mampu_pendidikan_detail pkd');
		$this->db->where('pkd.pk',$pk);
		$query = $this->db->get();
		...
		foreach ($data as &$item) {
			$item['index'] = $index;
			// $item['jenis_kelamin'] = $gender[$item['jk']];
			$index +=1;
		}

		return $data;
	}
	function delete_data_detail($pk=0)
	{
		$this->db->where('pk',$pk);
		$this->db->delete('t_sk_tidak_mampu_pendidikan_detail');
	}

}//D:/xampp/htdocs/babakan/app/modules/sk_tidak_mampu_pendidikan/models/M_sk_tidak_mampu_pendidikan.php
