...
class Sk_tidak_mampu_pendidikan extends Crud_Controller
{
    ...
    protected function _afterAddEditData($pk, $entry){
        ...
        if(!empty($post_detail_nama)){
            ...
            if(  $count_detail_nama != 0 ){
                ...
                if(count($entry2)>0){
                    $this->db->insert_batch('t_sk_tidak_mampu_pendidikan_detail',$entry2);
                }
            }
            $entry2 = array();
...
$gol_darah      = array(''=>'- - - Pilih Golongan Darah - - -');
    ...
    {
        $sk_tidak_mampu_pendidikanid = $this->input->post('sk_tidak_mampu_pendidikanid');
        $detail = $this->m_sk_tidak_mampu_pendidikan->detail_data($sk_tidak_mampu_pendidikanid);
        $anggota = $this->m_sk_tidak_mampu_pendidikan->detail_data_anggota($sk_tidak_mampu_pendidikanid);
        	$kel  = kel_3_dropdown($this->no_prop,$this->no_kab,$this->no_kec,$this->no_kel);
	$agama       = agama_2_dropdown();
	$pekerjaan   = pekerjaan_dropdown();
	$status_perkawinan  = status_kawin_2_dropdown();
...
$gol_darah      = array(''=>'- - - Pilih Golongan Darah - - -');
	...
	'syarat_form' => $this->syarat->form($detail['syarat']),
        	...
        	'sk_tidak_mampu_pendidikanid' => $sk_tidak_mampu_pendidikanid,
        	'detail' => $detail,
            'anggota' => $anggota ,
    'status_keluarga'=>$status_keluarga,
    ...
    public function download()
    {
        ...
        $options['rowModifierCallback'] = function ($row){
            ...
            $camat = $this->m_sk_tidak_mampu_pendidikan->get_camat();
            ...
            $ret['anggota_keluarga'] = $this->m_sk_tidak_mampu_pendidikan->detail_data_anggota($row['id']);
...
}//D:/xampp/htdocs/babakan/app/modules/sk_tidak_mampu_pendidikan/controllers/Sk_tidak_mampu_pendidikan.php
