...
class M_domisili_wni extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_domisili_wni pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_domisili_wni',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_domisili_wni',$data);
		}
	}
...
}//D:/xampp/htdocs/babakan/app/modules/domisili_wni/models/M_domisili_wni.php
