...
function upload_data(domisili_wniid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {domisili_wniid:domisili_wniid},
		cache : false,
		dataType : "html",
...
function edit_data(domisili_wniid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_domisili_wni').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var domisili_wniid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+domisili_wniid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(domisili_wniid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(domisili_wniid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(domisili_wniid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			domisili_wniid : domisili_wniid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
