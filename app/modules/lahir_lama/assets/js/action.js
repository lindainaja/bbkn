...
function upload_data(lahir_lamaid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {lahir_lamaid:lahir_lamaid},
		cache : false,
		dataType : "html",
...
function edit_data(lahir_lamaid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_lahir_lama').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var lahir_lamaid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+lahir_lamaid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(lahir_lamaid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(lahir_lamaid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(lahir_lamaid) {
	$.ajax({
		type : "POST",
