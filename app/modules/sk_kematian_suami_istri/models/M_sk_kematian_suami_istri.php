...
class M_sk_kematian_suami_istri extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_kematian_suami_istri pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_kematian_suami_istri',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_kematian_suami_istri',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_kematian_suami_istri');
	}
...
//D:/xampp/htdocs/babakan/app/modules/sk_kematian_suami_istri/models/M_sk_kematian_suami_istri.php
