...
function upload_data(sk_kematian_suami_istriid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_kematian_suami_istriid:sk_kematian_suami_istriid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_kematian_suami_istriid) {
	$.ajax({
		type : "POST",
...
function dialog(tpl_html,flag){
	...
	autocomplete_data_penduduk_hidup('#nama');
	autocomplete_data_penduduk_meninggal('#meninggal_nama');
	autocomplete_sebab_meninggal('#sebab_meninggal');
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_kematian_suami_istri').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_kematian_suami_istriid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_kematian_suami_istriid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_kematian_suami_istriid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_kematian_suami_istriid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_kematian_suami_istriid) {
	$.ajax({
		type : "POST",
...
function autocomplete_data_penduduk_hidup(identitas){
...
function autocomplete_data_penduduk_meninggal(identitas){
	$(identitas).autocomplete(base_url + "autocompletes/load_data_penduduk",
    { 
    	parse: function(data){
				if(data.length == null)
       ...
       extraParams: {
		...
		$("#meninggal_nama").val(data.nama);
		$("#meninggal_alias").val('-');
		$(".meninggal_jk").removeAttr("checked");
		$(".meninggal_jk_"+data.jk).attr("checked","checked");
		$("#meninggal_tempat_lahir").val(data.tmpt_lahir);
		$('#meninggal_tanggal_lahir').datepicker('setDate', format_tanggal_input(data.tgl_lahir));
		$("#meninggal_agama").val(data.agama).trigger("change");
		$("#meninggal_pekerjaan").val(data.pekerjaan).trigger("change");
		$("#meninggal_alamat").val(data.alamat);
		$("#meninggal_bin_binti").val(data.nama_ayah);
...
function autocomplete_sebab_meninggal(){
	$("#sebab_meninggal").autocomplete(base_url + "autocompletes/load_sebab_kematian",
    { 
    	parse: function(data){
				if(data.length == null)
				{
       ...
       extraParams: {
		   q: function () { return $("#sebab_meninggal").val() },
		   limit: 20,
     	},                                      
        formatItem: function(data,i,max)
        {
