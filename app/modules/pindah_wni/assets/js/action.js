...
function upload_data(pindah_wniid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {pindah_wniid:pindah_wniid},
		cache : false,
		dataType : "html",
...
function edit_data(pindah_wniid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form() && validasi_anggota()){
		var datas = $('#form_pindah_wni').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var pindah_wniid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+pindah_wniid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(pindah_wniid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(pindah_wniid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(pindah_wniid) {
	$.ajax({
		type : "POST",
...
function autocomplete_data_penduduk_kk(identitas){
    ...
    { 
       ...
       extraParams: {
		...
		var rt_rw=data.alamat.match(/RT\s+:\s+(\d+);RW\s+:\s+(\d+)/);
		console.log(rt_rw);
		data.rt_asal = rt_rw[1];
		data.rw_asal = rt_rw[2];
		$("#rt_asal").val(data.rt_asal);
		$("#rw_asal").val(data.rw_asal);
	});
}
