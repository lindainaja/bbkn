...
class M_pindah_wni extends CI_Model 
{
...
public function get_camat()
	{
		return $this->db->where('jabatan','Camat '.config_item('default_nama_kec'))->get('r_pegawai')->row_array();
	}
	...
	function detail_data($id=0,$r=0)
	{
		$this->db->select('pk.*',false);
		$this->db->select('sp.nama_prop,skab.nama_kab,skec.nama_kec,skel.nama_kel',false);
		$this->db->select('skec.nama_pej,skec.nip_pej',false);
		$this->db->select('skel.nama_lur,skel.nip_lur,skel.kodepos,skel.address as alamat_kel',false);
	...
	function detail_data_anggota($pindah_wniid=0)
	{
		$this->db->select('pkd.*',false);
		$this->db->from('t_pindah_wni_detail pkd');
		$this->db->where('pkd.pindah_wniid',$pindah_wniid);
		$query = $this->db->get();
	...
	{
		if(count($data)>0){
			$this->db->insert('t_pindah_wni',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_pindah_wni',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_pindah_wni');
	}

	function delete_data_detail($pindah_wniid=0)
	{
		$this->db->where('pindah_wniid',$pindah_wniid);
		$this->db->delete('t_pindah_wni_detail');
	}
