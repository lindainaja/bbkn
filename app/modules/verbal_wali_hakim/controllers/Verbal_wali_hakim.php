...
class Verbal_wali_hakim extends Crud_Controller{
    ...
    protected function _preAddEditData(&$entry){
        for($i=1;$i<6;$i++){
            $entry['sebab_'.$i]         = $this->input->post('sebab_'.$i);
        }
    }
    ...
    {
        $verbal_wali_hakimid = $this->input->post('verbal_wali_hakimid');
        $detail = $this->m_verbal_wali_hakim->detail_data($verbal_wali_hakimid);
        $negara		 	= negara_dropdown();
        $agama 			= agama_2_dropdown();
        $pekerjaan	 	= pekerjaan_dropdown();
        $kel		 	= kel_3_dropdown($this->no_prop,$this->no_kab,$this->no_kec,$this->no_kel);
...
//D:/xampp/htdocs/babakan/app/modules/verbal_wali_hakim/controllers/Verbal_wali_hakim.php
