...
class M_verbal_wali_hakim extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_verbal_wali_hakim pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_verbal_wali_hakim',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_verbal_wali_hakim',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_verbal_wali_hakim');
	}
