...
function upload_data(sk_persetujuan_mempelaiid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_persetujuan_mempelaiid:sk_persetujuan_mempelaiid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_persetujuan_mempelaiid) {
	$.ajax({
		type : "POST",
...
function dialog(tpl_html,flag){
	...
	autocomplete_data_penduduk('#cs_nama');
	autocomplete_data_penduduk('#ci_nama');
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_persetujuan_mempelai').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_persetujuan_mempelaiid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_persetujuan_mempelaiid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_persetujuan_mempelaiid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_persetujuan_mempelaiid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_persetujuan_mempelaiid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			sk_persetujuan_mempelaiid : sk_persetujuan_mempelaiid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
...
function autocomplete_data_penduduk(identitas){
    ...
    { 
       ...
       extraParams: {
		...
		if(id.match(/\_/)){
			id = id.split('_')[0];
			$("#"+id+"_tempat_lahir").val(data.tmpt_lahir);
			$('#'+id+'_tanggal_lahir').datepicker('setDate', format_tanggal_input(data.tgl_lahir));
			$("#"+id+"_agama").val(data.agama).trigger("change");
			$("#"+id+"_pekerjaan").val(data.pekerjaan).trigger("change");
			$("#"+id+"_tempat_tinggal").val(data.alamat);
			$("#"+id+"kewarganegaraan").val(data.kewarganegaraan).trigger("change");

			var bt=id=='cs'?'bin':'binti';
			$("#"+id+"_"+bt).val(data.nama_ayah);
		}else{
			...
			$("#tempat_tinggal").val(data.alamat);
