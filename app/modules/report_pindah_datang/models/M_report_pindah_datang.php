...
class M_report_pindah_datang extends CI_Model 
{
	...
	public function get_pelayanan_pindat($no_kel,$month,$year,$jenis='BARU')
	{
		$table_sufixes=['t_datang','t_pindah',108,126,128,129,130,131,132,'134_133','136_135',137,138,139];
		$captions = ['datang'=>'Kedatangan','pindah'=>'Kepindahan'];
		foreach ($table_sufixes as $suf) {
			$table_name = 't_pindah_wni_'.$suf;
			$key= 'Form '.extract_f($suf);

			if(preg_match('/^t_/', $suf)){
				$key = str_replace('t_', '', $suf);
