...
class M_sk_ahli_waris_tanah extends CI_Model 
{
	...
	{
		return $this->db->select('k.*, k.nama_pej nama_pegawai,k.nip_pej nip')->where('no_kec',$this->no_kec)->get('setup_kec k')->row_array();
	}
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_sk_ahli_waris_tanah pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_sk_ahli_waris_tanah',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_sk_ahli_waris_tanah',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_sk_ahli_waris_tanah');
	}
...
}//D:/xampp/htdocs/babakan/app/modules/sk_ahli_waris_tanah/models/M_sk_ahli_waris_tanah.php
