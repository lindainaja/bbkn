...
function upload_data(sk_ahli_waris_tanahid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_ahli_waris_tanahid:sk_ahli_waris_tanahid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_ahli_waris_tanahid) {
	$.ajax({
		type : "POST",
...
function dialog(tpl_html,flag){
	...
	autocomplete_data_aw('#aw1_nama');
	autocomplete_data_aw('#aw2_nama');
	autocomplete_data_aw('#aw3_nama');
	autocomplete_data_aw('#aw4_nama');

	autocomplete_data_al('#nama_almarhum');
	autocomplete_data_al('#nama_pasangan_almarhum');

	autocomplete_data_al('#saksi_1');
	autocomplete_data_al('#saksi_2');

	autocomplete_ttd('#ttd_nama');
	autocomplete_ttd('#ttd_nip');

	autocomplete_ttd_kl('#ttd_kl_nama');
	autocomplete_ttd_kl('#ttd_kl_nip');
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_ahli_waris_tanah').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_ahli_waris_tanahid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_ahli_waris_tanahid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_ahli_waris_tanahid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_ahli_waris_tanahid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_ahli_waris_tanahid) {
	$.ajax({
		type : "POST",
...
// 
function autocomplete_data_al(identitas){
...
function autocomplete_data_aw(identitas){
	var prefix = identitas.split('_')[0];
	console.log(prefix);
	$(identitas).autocomplete(base_url + "autocompletes/load_data_penduduk",
    { 
    	parse: function(data){
       ...
       extraParams: {
		...
		$(prefix+"_nama").val(data.nama);
		$(prefix+"_status").val(data.shdk).trigger('change');
		// $(prefix+"_jk_"+data.jk).attr("checked","checked");
		$(prefix+"_tempat_lahir").val(data.tmpt_lahir);
		$(prefix+'_tanggal_lahir').datepicker('setDate', format_tanggal_input(data.tgl_lahir));
		$(prefix+"_agama").val(data.agama).trigger("change");
		$(prefix+"_pekerjaan").val(data.pekerjaan).trigger("change");
		$(prefix+"_alamat").val(data.alamat);
...
function autocomplete_ttd_kl(identitas){
	$(identitas).autocomplete(base_url + "autocompletes/load_data_tdd",
    { 
    	parse: function(data){
				if(data.length == null)
       ...
       extraParams: {
		...
		$("#ttd_kl_nama").val(data.nama_pegawai);
		$("#ttd_kl_nip").val(data.nip_baru);
		$("#ttd_kl_jabatan").val(data.nama_jabatan);
