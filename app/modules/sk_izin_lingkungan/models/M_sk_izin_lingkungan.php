...
class M_sk_izin_lingkungan extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('sk_izin_lingkungan pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('sk_izin_lingkungan',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('sk_izin_lingkungan',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('sk_izin_lingkungan');
	}
...
}//D:/xampp/htdocs/babakan/app/modules/sk_izin_lingkungan/models/M_sk_izin_lingkungan.php
