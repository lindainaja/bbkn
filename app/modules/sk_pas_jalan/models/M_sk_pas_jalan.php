...
class M_sk_pas_jalan extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_sk_pas_jalan pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_sk_pas_jalan',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_sk_pas_jalan',$data);
		}
	}
...
}//D:/xampp/htdocs/babakan/app/modules/sk_pas_jalan/models/M_sk_pas_jalan.php
