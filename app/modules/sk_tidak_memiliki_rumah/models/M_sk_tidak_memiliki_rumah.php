...
class M_sk_tidak_memiliki_rumah extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_tidak_memiliki_rumah pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_tidak_memiliki_rumah',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_tidak_memiliki_rumah',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_tidak_memiliki_rumah');
	}
...
}//D:/xampp/htdocs/babakan/app/modules/sk_tidak_memiliki_rumah/models/M_sk_tidak_memiliki_rumah.php
