...
function upload_data(sk_duda_jandaid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_duda_jandaid:sk_duda_jandaid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_duda_jandaid) {
	$.ajax({
		type : "POST",
...
function dialog(tpl_html,flag){
	...
	autocomplete_data_penduduk_dari('#dari_nama');
...
function default_pernyataan(){
	var status = $(".status:checked").val();
	if(status=='DUDA'){
		var text = 'Bahwa Istrinya tersebut diatas telah menikah Lagi  serta Surat Keterangan  Duda (Cerai Hidup) yang dimiliknya belum di ambil Di Pengadilan Agama Kota Tangerang';
	} else {
		var text = '';
	}
	$("#keterangan").val(text);
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_duda_janda').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_duda_jandaid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_duda_jandaid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_duda_jandaid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_duda_jandaid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_duda_jandaid) {
	$.ajax({
		type : "POST",
...
function autocomplete_data_penduduk_dari(identitas){
    ...
    { 
       ...
       extraParams: {
		...
		$("#dari_nama").val(data.nama);
		$("#dari_tempat_lahir").val(data.tmpt_lahir);
		$('#dari_tanggal_lahir').datepicker('setDate', format_tanggal_input(data.tgl_lahir));
		$("#dari_agama").val(data.agama).trigger("change");
		$("#dari_pekerjaan").val(data.pekerjaan).trigger("change");
		$("#dari_alamat").val(data.alamat);
