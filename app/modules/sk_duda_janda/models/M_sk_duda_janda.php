...
class M_sk_duda_janda extends CI_Model 
{
	...
	{
		if(count($data)>0){
			$this->db->insert('t_duda_janda',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_duda_janda',$data);
		}
	}
