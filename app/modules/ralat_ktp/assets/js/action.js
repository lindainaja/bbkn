...
function upload_data(ralat_ktpid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {ralat_ktpid:ralat_ktpid},
		cache : false,
		dataType : "html",
...
function edit_data(ralat_ktpid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	...
	if(no_kel == ''){
		...
		alert_warning("Silahkan mengisi kolom ralat nik 1","#nik_1");
	} else if(nik_2 == ''){
		alert_warning("Silahkan mengisi kolom ralat nik 2","#nik_2");
	} else if(ttd_nama == ''){
		...
		var datas = $('#form_ralat_ktp').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var ralat_ktpid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+ralat_ktpid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(ralat_ktpid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(ralat_ktpid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(ralat_ktpid) {
	$.ajax({
		type : "POST",
