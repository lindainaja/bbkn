...
function upload_data(sk_domisili_kkid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_domisili_kkid:sk_domisili_kkid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_domisili_kkid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_domisili_kk').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_domisili_kkid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_domisili_kkid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_domisili_kkid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_domisili_kkid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_domisili_kkid) {
	$.ajax({
		type : "POST",
...
function add_row(){
	...
	var tpl_html = '<div class="form-group anggota_keluarga" id="row_'+rand+'">';
		...
		tpl_html += '<div class="col-sm-3 col-lg-3 controls">';
		tpl_html += '<select name="detail_jk[]" id="detail_jk_'+rand+'" flag="'+rand+'" class="select2 form-control" style="width: 100%;">'+tpl_option_2+'</select>';
		tpl_html += '</div>';
		tpl_html += '<div class="col-sm-3 col-lg-3 controls">';
