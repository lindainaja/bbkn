...
class Sk_domisili_kk extends Crud_Controller
{
    ...
    $jenis_kelamin_option = array_merge(array(''=>'--Jenis Kelamin--'),config_item('gender'));
	...
	'jenis_kelamin' => $jenis_kelamin_option,
    ...
    {
        $sk_domisili_kkid = $this->input->post('sk_domisili_kkid');
        $detail = $this->m_sk_domisili_kk->detail_data($sk_domisili_kkid);
        $detail_data_anggota = $this->m_sk_domisili_kk->detail_data_anggota($sk_domisili_kkid);
        	$kel  = kel_3_dropdown($this->no_prop,$this->no_kab,$this->no_kec,$this->no_kel);
	$status_perkawinan  = status_kawin_2_dropdown();
	$pekerjaan   = pekerjaan_dropdown();
	$agama       = agama_2_dropdown();
	...
	'syarat_form' => $this->syarat->form($detail['syarat']),
        	...
        	'sk_domisili_kkid' => $sk_domisili_kkid,
        	'detail' => $detail,
            'anggota' => $detail_data_anggota 
        );
    ...
    public function download()
    {
        ...
        $options['rowModifierCallback'] = function ($row){
            ...
            $ret['anggota_keluarga'] = $this->{$this->modelName}->detail_data_anggota($row['id']);
    ...
    protected function _afterAddEditData($pk, $entry){
        ...
        if(!empty($post_detail_nama)){
            ...
            if(  $count_detail_nama != 0 ){
                ...
                if(count($entry2)>0){
                    $this->db->insert_batch('t_sk_domisili_kk_detail',$entry2);
                }
            }
            $entry2 = array();
...
}//D:/xampp/htdocs/babakan/app/modules/sk_domisili_kk/controllers/Sk_domisili_kk.php
