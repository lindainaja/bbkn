...
function upload_data(sk_domisili_yayasanid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_domisili_yayasanid:sk_domisili_yayasanid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_domisili_yayasanid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_domisili_yayasan').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_domisili_yayasanid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_domisili_yayasanid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_domisili_yayasanid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_domisili_yayasanid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_domisili_yayasanid) {
	$.ajax({
		type : "POST",
