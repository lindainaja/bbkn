...
class M_sk_domisili_yayasan extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('sk_domisili_yayasan pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('sk_domisili_yayasan',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('sk_domisili_yayasan',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('sk_domisili_yayasan');
	}
...
}//D:/xampp/htdocs/babakan/app/modules/sk_domisili_yayasan/models/M_sk_domisili_yayasan.php
