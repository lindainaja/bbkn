...
function upload_data(surat_pernyataanid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {surat_pernyataanid:surat_pernyataanid},
		cache : false,
		dataType : "html",
...
function edit_data(surat_pernyataanid) {
	$.ajax({
		type : "POST",
...
function dialog(tpl_html,flag){
	...
	autocomplete_data_penduduk_pengaju('#nama');
	autocomplete_data_penduduk_tertuju('#tertuju_nama');
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_surat_pernyataan').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var surat_pernyataanid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+surat_pernyataanid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(surat_pernyataanid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(surat_pernyataanid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(surat_pernyataanid) {
	$.ajax({
		type : "POST",
...
function autocomplete_data_penduduk_pengaju(identitas){
...
function autocomplete_data_penduduk_tertuju(identitas){
	$(identitas).autocomplete(base_url + "autocompletes/load_data_penduduk",
    { 
    	parse: function(data){
				if(data.length == null)
       ...
       extraParams: {
		...
		$("#tertuju_nama").val(data.nama);
		$("#tertuju_tempat_lahir").val(data.tmpt_lahir);
		$('#tertuju_tanggal_lahir').datepicker('setDate', format_tanggal_input(data.tgl_lahir));
		$("#tertuju_agama").val(data.agama).trigger("change");
		$("#tertuju_pekerjaan").val(data.pekerjaan).trigger("change");
		$("#tertuju_alamat").val(data.alamat);
