...
function upload_data(sk_imbid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_imbid:sk_imbid},
		cache : false,
		dataType : "html",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_imb').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_imbid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_imbid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_imbid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_imbid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_imbid) {
	$.ajax({
		type : "POST",
