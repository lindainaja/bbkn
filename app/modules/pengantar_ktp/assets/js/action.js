...
function upload_data(pengantar_ktpid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {pengantar_ktpid:pengantar_ktpid},
		cache : false,
		dataType : "html",
...
function edit_data(pengantar_ktpid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	...
	if(no_kel == ''){
		...
		alert_warning("Silahkan masukkan nik","#nik");
	} else if(alamat == ''){
		...
		var datas = $('#form_pengantar_ktp').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var pengantar_ktpid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+pengantar_ktpid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(pengantar_ktpid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(pengantar_ktpid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(pengantar_ktpid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			pengantar_ktpid : pengantar_ktpid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
