...
class M_pengantar_ktp extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_pengantar_ktp pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_pengantar_ktp',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_pengantar_ktp',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_pengantar_ktp');
	}
