...
function upload_data(perubahan_kkid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {perubahan_kkid:perubahan_kkid},
		cache : false,
		dataType : "html",
...
function add_row(){
	...
	var tpl_html = '<div class="form-group anggota_keluarga" id="row_'+rand+'">';
		...
		tpl_html += '</div>';
		tpl_html += '<div class="col-sm-3 col-lg-4 controls">';
...
function edit_data(perubahan_kkid) {
	$.ajax({
		type : "POST",
...
function dialog(tpl_html,flag){
	...
	autocomplete_data_penduduk('#nama_kk');
...
function process_add_edit_data(status){
	if(validasi_form() && validasi_anggota()){
		var datas = $('#form_perubahan_kk').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var perubahan_kkid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+perubahan_kkid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(perubahan_kkid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(perubahan_kkid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(perubahan_kkid) {
	$.ajax({
		type : "POST",
...
function autocomplete_data_penduduk(identitas){
    ...
    { 
       ...
       extraParams: {
		...
		$("#nama_kk_lama").val(data.nama);
		$("#nik_lama").val(data.nik);
		$("#no_kk_lama").val(data.no_kk);
		$("#alamat_lama").val(data.alamat);

		$("#kel_lama").val(data.nama_kel);
		$("#kec_lama").val(data.nama_kec);
		$("#kab_lama").val(data.nama_kab);
		$("#prop_lama").val(data.nama_prop);
