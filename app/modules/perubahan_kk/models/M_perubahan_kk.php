...
class M_perubahan_kk extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_perubahan_kk pk');
	...
	function detail_data_anggota($perubahan_kkid=0)
	{
		$this->db->select('pkd.*',false);
		$this->db->from('t_perubahan_kk_detail pkd');
		$this->db->where('pkd.perubahan_kkid',$perubahan_kkid);
		$query = $this->db->get();
	...
	{
		if(count($data)>0){
			$this->db->insert('t_perubahan_kk',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_perubahan_kk',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_perubahan_kk');
	}

	function delete_data_detail($perubahan_kkid=0)
	{
		$this->db->where('perubahan_kkid',$perubahan_kkid);
		$this->db->delete('t_perubahan_kk_detail');
	}
