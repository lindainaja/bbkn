...
function upload_data(datangid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {datangid:datangid},
		cache : false,
		dataType : "html",
...
function process_add_edit_data(status){
	if(validasi_form() && validasi_anggota()){
		var datas = $('#form_datang').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var datangid = data.id
					window.location = base_url + '/'+controller+'/download/id/'+datangid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(datangid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(datangid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(datangid) {
	$.ajax({
		type : "POST",
