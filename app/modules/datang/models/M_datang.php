...
class M_datang extends CI_Model 
{
...
public function get_camat()
	...
	function detail_data_anggota($datangid=0)
	{
		$this->db->select('pkd.*',false);
		$this->db->from('t_datang_detail pkd');
		$this->db->where('pkd.datangid',$datangid);
		$query = $this->db->get();
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_datang');
	}

	function delete_data_detail($datangid=0)
	{
		$this->db->where('datangid',$datangid);
		$this->db->delete('t_datang_detail');
	}
