...
function upload_data(domisili_usahaid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {domisili_usahaid:domisili_usahaid},
		cache : false,
		dataType : "html",
...
function edit_data(domisili_usahaid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_domisili_usaha').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var domisili_usahaid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+domisili_usahaid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(domisili_usahaid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(domisili_usahaid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(domisili_usahaid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			domisili_usahaid : domisili_usahaid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
