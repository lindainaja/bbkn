...
class M_domisili_usaha extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_domisili_usaha pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_domisili_usaha',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_domisili_usaha',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_domisili_usaha');
	}
