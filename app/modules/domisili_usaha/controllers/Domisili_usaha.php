...
class Domisili_usaha extends Crud_Controller{
    ...
    protected function _preDownload(&$data){
        ...
        if(!empty($detail['nik']) && !empty($detail['pasport'])){
            $detail['nik_pasport'] = $detail['nik'].'/'.$detail['pasport'];
        }
        if(empty($detail['nik']) && !empty($detail['pasport'])){
            $detail['nik_pasport'] =  $detail['pasport'];
        }
        if(!empty($detail['nik']) && empty($detail['pasport'])){
            $detail['nik_pasport'] = $detail['nik'];
        }
...
$this->twig->display('form/add.twig.php',$data);
    ...
    {
        $domisili_usahaid = $this->input->post('domisili_usahaid');
$detail = $this->m_domisili_usaha->detail_data($domisili_usahaid);
$negara		 	= negara_dropdown();
$agama 			= agama_2_dropdown();
$pekerjaan	 	= pekerjaan_dropdown();
$kel		 	= kel_3_dropdown($this->no_prop,$this->no_kab,$this->no_kec,$this->no_kel);
...
//D:/xampp/htdocs/babakan/app/modules/domisili_usaha/controllers/Domisili_usaha.php
