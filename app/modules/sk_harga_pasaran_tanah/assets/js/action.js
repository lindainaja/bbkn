...
function upload_data(sk_harga_pasaran_tanahid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_harga_pasaran_tanahid:sk_harga_pasaran_tanahid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_harga_pasaran_tanahid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_harga_pasaran_tanah').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_harga_pasaran_tanahid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_harga_pasaran_tanahid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_harga_pasaran_tanahid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_harga_pasaran_tanahid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_harga_pasaran_tanahid) {
	$.ajax({
		type : "POST",
