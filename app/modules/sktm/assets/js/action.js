...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sktm').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sktmid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sktmid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sktmid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sktmid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sktmid) {
	$.ajax({
		type : "POST",
