...
function upload_data(domisili_pendudukid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {domisili_pendudukid:domisili_pendudukid},
		cache : false,
		dataType : "html",
...
function edit_data(domisili_pendudukid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_domisili_penduduk').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var domisili_pendudukid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+domisili_pendudukid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(domisili_pendudukid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(domisili_pendudukid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(domisili_pendudukid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			domisili_pendudukid : domisili_pendudukid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
...
function autocomplete_data_penduduk(identitas){
    ...
    { 
       ...
       extraParams: {
		...
		// $('.select2').select2();
		data.agama = ucfirst(data.agama,1);
		console.log(data)
		// data.agama=data.agama.toLowerCase().charAt(0).toUpperCase() + data.agama.slice(1);
		...
		$("#tanggal_lahir").val(data.tgl_lahir).datepicker('setDate', format_tanggal_input(data.tgl_lahir));
