...
class M_surat_keterangan_lain extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('surat_keterangan_lain pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('surat_keterangan_lain',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('surat_keterangan_lain',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('surat_keterangan_lain');
	}
...
}//D:/xampp/htdocs/babakan/app/modules/surat_keterangan_lain/models/M_surat_keterangan_lain.php
