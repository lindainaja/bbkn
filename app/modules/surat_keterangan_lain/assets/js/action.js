...
function upload_data(surat_keterangan_lainid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {surat_keterangan_lainid:surat_keterangan_lainid},
		cache : false,
		dataType : "html",
...
function edit_data(surat_keterangan_lainid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_surat_keterangan_lain').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var surat_keterangan_lainid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+surat_keterangan_lainid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(surat_keterangan_lainid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(surat_keterangan_lainid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(surat_keterangan_lainid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			surat_keterangan_lainid : surat_keterangan_lainid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
