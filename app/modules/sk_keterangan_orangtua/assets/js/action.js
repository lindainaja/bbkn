...
function upload_data(sk_keterangan_orangtuaid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_keterangan_orangtuaid:sk_keterangan_orangtuaid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_keterangan_orangtuaid) {
	$.ajax({
		type : "POST",
...
function dialog(tpl_html,flag){
	...
	autocomplete_data_penduduk_calon('#c_nama');
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_keterangan_orangtua').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_keterangan_orangtuaid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_keterangan_orangtuaid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_keterangan_orangtuaid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_keterangan_orangtuaid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_keterangan_orangtuaid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			sk_keterangan_orangtuaid : sk_keterangan_orangtuaid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
...
function autocomplete_data_penduduk_calon(identitas){
    ...
    { 
       ...
       extraParams: {
		...
		$("#c_alias").val('-');
		$("input[name=ci_jk]").find('.jk').removeAttr("checked");
		$("input[name=ci_jk]").find(".jk_"+data.jk).attr("checked","checked");
		$("#c_tempat_lahir").val(data.tmpt_lahir);
		$('#c_tanggal_lahir').datepicker('setDate', format_tanggal_input(data.tgl_lahir));
		$("#c_agama").val(data.agama).trigger("change");
		$("#c_pekerjaan").val(data.pekerjaan).trigger("change");
...
function autocomplete_data_penduduk_ibu(identitas){
    ...
    { 
       ...
       extraParams: {
		...
		// $("#bin_binti").val(data.nama_ayah);
		$("#ibu_nama").val(data.nama);
		// $(".jk").removeAttr("checked");
		// $(".jk_"+data.jk).attr("checked","checked");
		$("#ibu_tempat_lahir").val(data.tmpt_lahir);
		$('#ibu_tanggal_lahir').datepicker('setDate', format_tanggal_input(data.tgl_lahir));
		$("#ibu_agama").val(data.agama).trigger("change");
		$("#ibu_pekerjaan").val(data.pekerjaan).trigger("change");
		$("#ibu_tempat_tinggal").val(data.alamat);
...
function autocomplete_data_penduduk_ayah(identitas){
    ...
    { 
       ...
       extraParams: {
		...
		$("#ayah_tempat_lahir").val(data.tmpt_lahir);
		$('#ayah_tanggal_lahir').datepicker('setDate', format_tanggal_input(data.tgl_lahir));
		$("#ayah_agama").val(data.agama).trigger("change");
		$("#ayah_pekerjaan").val(data.pekerjaan).trigger("change");
		$("#ayah_tempat_tinggal").val(data.alamat);
		// $("#ayah_bin_binti").val(data.nama_ayah);
