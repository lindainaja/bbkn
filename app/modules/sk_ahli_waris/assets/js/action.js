...
function upload_data(sk_ahli_warisid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_ahli_warisid:sk_ahli_warisid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_ahli_warisid) {
	$.ajax({
		type : "POST",
...
function dialog(tpl_html,flag){
	...
	autocomplete_data_pd('#nama_almarhum');
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_ahli_waris').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_ahli_warisid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_ahli_warisid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_ahli_warisid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_ahli_warisid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_ahli_warisid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			sk_ahli_warisid : sk_ahli_warisid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
...
function autocomplete_data_pd(identitas){
