...
function upload_data(ikrar_masuk_islamid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {ikrar_masuk_islamid:ikrar_masuk_islamid},
		cache : false,
		dataType : "html",
...
function edit_data(ikrar_masuk_islamid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_ikrar_masuk_islam').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var ikrar_masuk_islamid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+ikrar_masuk_islamid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(ikrar_masuk_islamid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(ikrar_masuk_islamid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(ikrar_masuk_islamid) {
	$.ajax({
		type : "POST",
