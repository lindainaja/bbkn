...
class M_ikrar_masuk_islam extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_ikrar_masuk_islam pk');
	...
	function detail_data($id=0)
	{
		$this->db->select('pk.*',false);
		$this->db->select('sp.nama_prop,skab.nama_kab,skec.nama_kec,skel.nama_kel',false);
		$this->db->select('skec.nama_pej,skec.nip_pej,skec.address as alamat_kec',false);
	...
	{
		if(count($data)>0){
			$this->db->insert('t_ikrar_masuk_islam',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_ikrar_masuk_islam',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_ikrar_masuk_islam');
	}
