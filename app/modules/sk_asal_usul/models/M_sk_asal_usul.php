...
class M_sk_asal_usul extends CI_Model 
{
	...
	{
		if(count($data)>0){
			$this->db->insert('t_asal_usul',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_asal_usul',$data);
		}
	}
