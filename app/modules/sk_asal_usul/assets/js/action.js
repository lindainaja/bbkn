...
function upload_data(sk_asal_usulid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_asal_usulid:sk_asal_usulid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_asal_usulid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_asal_usul').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_asal_usulid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_asal_usulid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_asal_usulid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_asal_usulid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_asal_usulid) {
	$.ajax({
		type : "POST",
