...
function upload_data(pernyataan_ceraiid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {pernyataan_ceraiid:pernyataan_ceraiid},
		cache : false,
		dataType : "html",
...
function edit_data(pernyataan_ceraiid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_pernyataan_cerai').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var pernyataan_ceraiid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+pernyataan_ceraiid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(pernyataan_ceraiid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(pernyataan_ceraiid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(pernyataan_ceraiid) {
	$.ajax({
		type : "POST",
