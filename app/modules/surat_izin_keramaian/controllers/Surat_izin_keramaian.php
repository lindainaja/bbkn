...
class Surat_izin_keramaian extends Crud_Controller{
...
$this->twig->display('form/add.twig.php',$data);;
    ...
    {
        // $golongan_darah = array(''=>'- - -  Pilih Gol Darah  - - -')+ config_item('gol_darah') + array('Tidak Tahu'=>'Tidak Tahu');
        $surat_izin_keramaianid = $this->input->post('surat_izin_keramaianid');
$detail = $this->m_surat_izin_keramaian->detail_data($surat_izin_keramaianid);
// $status_perkawinan	= status_kawin_2_dropdown();
$negara		 	= negara_dropdown();
$agama 			= agama_2_dropdown();
$pekerjaan	 	= pekerjaan_dropdown();
...
$this->twig->display('form/edit.twig.php',$data);;
     ...
     public function download()
    {
        ...
        $options['rowModifierCallback'] = function ($row){
            ...
            $ret['tanggal_acara'] = format_tanggal_khusus_indo($row['tgl_acara']);
            $ret['hari_acara'] = get_nama_hari($row['tgl_acara']);
            $ret['nama_acara'] = $ret['acara'];
            $ret['waktu_acara'] = date('H:i',strtotime($row['date'].' '.$ret['jam_acara_mulai'])) .'-'. date('H:i',strtotime($row['date'].' '.$ret['jam_acara_selesai'])) . ' WIB.';
...
//D:/xampp/htdocs/babakan/app/modules/surat_izin_keramaian/controllers/Surat_izin_keramaian.php
