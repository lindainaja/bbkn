...
function upload_data(sk_kematianid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_kematianid:sk_kematianid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_kematianid) {
	$.ajax({
		type : "POST",
...
function dialog(tpl_html,flag){
	...
	autocomplete_data_penduduk('#nama_pelapor');
	autocomplete_sebab_meninggal('#sebab_kematian')
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_kematian').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_kematianid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_kematianid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_kematianid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_kematianid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_kematianid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			sk_kematianid : sk_kematianid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
...
function autocomplete_sebab_meninggal(identitas){
	$(identitas).autocomplete(base_url + "autocompletes/load_sebab_kematian",
    { 
    	parse: function(data){
				if(data.length == null)
				{
