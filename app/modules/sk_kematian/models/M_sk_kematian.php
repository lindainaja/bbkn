...
class M_sk_kematian extends CI_Model 
{
	...
	{
		if(count($data)>0){
			$this->db->insert('t_sk_kematian',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_sk_kematian',$data);
		}
	}
...
}//D:/xampp/htdocs/babakan/app/modules/sk_kematian/models/M_sk_kematian.php
