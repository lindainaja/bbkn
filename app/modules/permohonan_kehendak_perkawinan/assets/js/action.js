...
function upload_data(permohonan_kehendak_perkawinanid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {permohonan_kehendak_perkawinanid:permohonan_kehendak_perkawinanid},
		cache : false,
		dataType : "html",
...
function edit_data(permohonan_kehendak_perkawinanid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_permohonan_kehendak_perkawinan').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var permohonan_kehendak_perkawinanid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+permohonan_kehendak_perkawinanid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(permohonan_kehendak_perkawinanid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(permohonan_kehendak_perkawinanid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(permohonan_kehendak_perkawinanid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			permohonan_kehendak_perkawinanid : permohonan_kehendak_perkawinanid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
