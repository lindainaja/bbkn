...
class M_sk_belum_pernah_nikah extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_belum_pernah_nikah pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_belum_pernah_nikah',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_belum_pernah_nikah',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_belum_pernah_nikah');
	}
