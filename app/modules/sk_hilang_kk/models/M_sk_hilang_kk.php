...
class M_sk_hilang_kk extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_sk_hilang_kk pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_sk_hilang_kk',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_sk_hilang_kk',$data);
		}
	}
...
}//D:/xampp/htdocs/babakan/app/modules/sk_hilang_kk/models/M_sk_hilang_kk.php
