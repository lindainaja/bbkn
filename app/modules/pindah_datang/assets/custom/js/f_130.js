...
gc.onShowForm = function(modal,mode,ajax_url,e){
	var pindah_antar_value = 'Antar Kecamatan Dalam Satu Kabupaten / Kota';
	...
	$exchange_field_config = [
		...
		['kec_asal','kel_asal',2],
		['rt_pindah','rw_pindah',2],
		['kodepos_pindah','tlp_pindah',2],
	...
	setTimeout(function(){
		...
		modal.find('input[name=pindah_antar_disabled]').attr('disabled',true).val(pindah_antar_value);
		modal.find('#field-no_kel').val(gc.default_no_kel);

		var extraParams = {
			nik:{
				no_kk: function(){return modal.find('#field-no_kk_asal').val();},
				f:'nik'
			},
			nama:{
				no_kk: function(){return modal.find('#field-no_kk_asal').val();},
		...
		gc.init_vm(modal,mode,"Keluarga Yang Pindah",extraParams);
