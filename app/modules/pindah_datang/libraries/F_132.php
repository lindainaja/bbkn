 ...
 * 
 */
class F_132  extends PD
{
	public $table_name = 't_pindah_wni_132';
	...
	{	
        ...
        $c->unset_read();
        $c->add_custom_form_css($this->asset_url('custom/css/f_132.css'));
        $c->add_custom_list_css($this->asset_url('custom/css/table.css'));
		$c->add_custom_form_js($this->asset_url('custom/js/f_132.js'));
        $c->set_subject('Data F-1.32');
        /*******************************************************************************/
    ...
    public function download_xlsx($pk)
    {
        ...
        $sheet->setCellValue('U12',$row['nama_kelurahan'])
              ...
              ->setCellValue('U14',$row['dusun_dukuh_kampung'])
        ...
        excel_write_char('K14', $row['no_dusun_dukuh_kampung'],$sheet,4,2); 
        ...
        excel_write_char('I50', $date['d'],$sheet,2,2);       
        excel_write_char('N50', $date['m'],$sheet,2,2);       
        excel_write_char('S50', $date['y'],$sheet,4,2);       
        ...
        $sheet->setCellValue("X71", $row['nama_kecamatan'] . ", " . $row['tanggal_surat']);
        
        $sheet->setCellValue('X77',$row['ttd_nama']);
        $sheet->setCellValue('X78','Nip. '.$row['ttd_nip']);
        if(preg_match('/camat/i',$row['ttd_jabatan'])){
            $sheet->setCellValue('X73',$row['ttd_jabatan']);
        }
        else if($row['ttd_nip'] != config_item('default_ttd_nip') || !empty($row['ttd_an_jabatan'])){
            $sheet->setCellValue('X73','An. ' . config_item('default_ttd_jabatan'));
            $sheet->setCellValue('X74',$row['ttd_jabatan']);
        }else{
            $sheet->setCellValue('X73',$row['ttd_jabatan']);
