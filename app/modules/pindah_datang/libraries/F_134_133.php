<?
class F_134_133  extends PD
{
	public $table_name = 't_pindah_wni_134_133';
	...
	{	
        ...
        $c->unset_read();
        $c->add_custom_form_css($this->asset_url('custom/css/f_134_133.css'));
        $c->add_custom_list_css($this->asset_url('custom/css/table.css'));
		$c->add_custom_form_js($this->asset_url('custom/js/f_134_133.js'));
        $c->set_subject('Data F-1.34 & 1.33');
        /*******************************************************************************/
    ...
    public function download_xlsx($pk)
    {
        ...
        $sheet->setCellValue('Y78',$row['nama_pemohon']);
        $sheet->setCellValue('B78',$row['nama_petugas_registrasi']);

        $sheet->setCellValue('U128',$row['ttd_jabatan']);
        $sheet->setCellValue('W132',$row['ttd_nama']);
        $sheet->setCellValue('T133','NIP. '.$row['ttd_nip'])
        //Q115-Q112-Q110-Q108-Q106-Q104-D97
              ->setCellValue('D97',$row['no'])
              ->setCellValue('Q104',$row['nik_pemohon'])
              ->setCellValue('Q106',$row['nama_pemohon'])
              ->setCellValue('Q108',$row['no_kk_asal'])
              ->setCellValue('Q110',$row['nama_kk_asal'])
              ->setCellValue('Q112',$row['alamat_asal'])
              ->setCellValue('Q115',$row['alamat_pindah']);
            
        //daftar-keleuaraga-Y78-B78-U126
        $row['tanggal_surat']  = format_tanggal_khusus_indo($row['date']);
        $sheet->setCellValue("X74", $row['nama_kelurahan'] . ", " . $row['tanggal_surat']);
        $sheet->setCellValue("U126", $row['nama_kelurahan'] . ", " . $row['tanggal_surat']);

        $daftar_keluarga_data = json_decode($row['daftar_keluarga_data']);
        //Q118
        $ag_cx = 0+count($daftar_keluarga_data);

        $sheet->setCellValue('Q118',$ag_cx);
