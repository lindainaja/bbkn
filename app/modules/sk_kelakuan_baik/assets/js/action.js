...
function upload_data(sk_kelakuan_baikid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_kelakuan_baikid:sk_kelakuan_baikid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_kelakuan_baikid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_kelakuan_baik').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_kelakuan_baikid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_kelakuan_baikid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_kelakuan_baikid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_kelakuan_baikid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_kelakuan_baikid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			sk_kelakuan_baikid : sk_kelakuan_baikid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
...
function autocomplete_data_penduduk(identitas){
    ...
    { 
       ...
       extraParams: {
		...
		$("#alamat_sekarang").val(data.alamat);
		$("#nik").val(data.nik);
