...
function upload_data(sk_wali_nikahid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_wali_nikahid:sk_wali_nikahid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_wali_nikahid) {
	$.ajax({
		type : "POST",
...
function dialog(tpl_html,flag){
	...
	autocomplete_data_penduduk_wali('#nama');
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_wali_nikah').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_wali_nikahid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_wali_nikahid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_wali_nikahid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_wali_nikahid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_wali_nikahid) {
	$.ajax({
		type : "POST",
...
function autocomplete_data_penduduk_wali(identitas){
