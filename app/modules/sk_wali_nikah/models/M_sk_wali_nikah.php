...
class M_sk_wali_nikah extends CI_Model 
{
	...
	{
		if(count($data)>0){
			$this->db->insert('t_wali_nikah',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_wali_nikah',$data);
		}
	}
