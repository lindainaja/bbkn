...
function upload_data(sk_kebakaranid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_kebakaranid:sk_kebakaranid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_kebakaranid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_kebakaran').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_kebakaranid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_kebakaranid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_kebakaranid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_kebakaranid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_kebakaranid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			sk_kebakaranid : sk_kebakaranid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
