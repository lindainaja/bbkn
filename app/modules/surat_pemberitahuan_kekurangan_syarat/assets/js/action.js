...
function upload_data(surat_pemberitahuan_kekurangan_syaratid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {surat_pemberitahuan_kekurangan_syaratid:surat_pemberitahuan_kekurangan_syaratid},
		cache : false,
		dataType : "html",
...
function edit_data(surat_pemberitahuan_kekurangan_syaratid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_surat_pemberitahuan_kekurangan_syarat').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var surat_pemberitahuan_kekurangan_syaratid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+surat_pemberitahuan_kekurangan_syaratid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(surat_pemberitahuan_kekurangan_syaratid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(surat_pemberitahuan_kekurangan_syaratid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(surat_pemberitahuan_kekurangan_syaratid) {
	$.ajax({
		type : "POST",
