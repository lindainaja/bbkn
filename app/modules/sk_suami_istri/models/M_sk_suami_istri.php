...
class M_sk_suami_istri extends CI_Model 
{
	...
	function detail_data($id=0,$result=0)
	{
		$this->db->select('pk.*',false);
		$this->db->select('sp.nama_prop,skab.nama_kab,skec.nama_kec,skel.nama_kel',false);
	...
	{
		if(count($data)>0){
			$this->db->insert('t_suami_istri',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_suami_istri',$data);
		}
	}
