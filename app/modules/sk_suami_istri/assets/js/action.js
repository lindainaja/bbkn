...
function upload_data(sk_suami_istriid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_suami_istriid:sk_suami_istriid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_suami_istriid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_suami_istri').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_suami_istriid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_suami_istriid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_suami_istriid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_suami_istriid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_suami_istriid) {
	$.ajax({
		type : "POST",
