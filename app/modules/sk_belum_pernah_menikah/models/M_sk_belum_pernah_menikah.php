...
class M_sk_belum_pernah_menikah extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('sk_belum_pernah_menikah pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('sk_belum_pernah_menikah',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('sk_belum_pernah_menikah',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('sk_belum_pernah_menikah');
	}
