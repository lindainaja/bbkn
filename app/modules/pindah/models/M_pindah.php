...
public function get_camat()
	...
	function detail_data_anggota($pindahid=0)
	{
		$this->db->select('pkd.*',false);
		$this->db->from('t_pindah_detail pkd');
		$this->db->where('pkd.pindahid',$pindahid);
		$query = $this->db->get();
	...
	function delete_data_detail($pindahid=0)
	{
		$this->db->where('pindahid',$pindahid);
		$this->db->delete('t_pindah_detail');
	}
