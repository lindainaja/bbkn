...
function upload_data(sk_ghoibid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_ghoibid:sk_ghoibid},
		cache : false,
		dataType : "html",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_ghoib').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_ghoibid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_ghoibid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_ghoibid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_ghoibid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_ghoibid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			sk_ghoibid : sk_ghoibid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
