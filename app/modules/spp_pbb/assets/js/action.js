...
function upload_data(spp_pbbid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {spp_pbbid:spp_pbbid},
		cache : false,
		dataType : "html",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_spp_pbb').serializeArray();
		console.log(datas);
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var spp_pbbid = data.id
					
					download_url= base_url + ''+controller+'/download/id/'+spp_pbbid;
...
function hapus_data(spp_pbbid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(spp_pbbid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(spp_pbbid) {
	$.ajax({
		type : "POST",
