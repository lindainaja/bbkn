...
class Spp_pbb extends Crud_Controller{
    ...
    protected function _preAddEditData(&$entry){
        $currency_fields = ['luas_tanah_asal','luas_bangunan','mutasi_luas_tanah','mutasi_luas_bangunan','pb_luas_tanah','pb_luas_bangunan'];

        foreach ($currency_fields as $key) {
            $entry[$key] = str_replace(',', '', $entry[$key]);
        }
...
$this->twig->display('form/add.twig.php',$data);
    ...
    {
        $spp_pbbid = $this->input->post('spp_pbbid');
$detail = $this->m_spp_pbb->detail_data($spp_pbbid);
$kel		 	= kel_3_dropdown($this->no_prop,$this->no_kab,$this->no_kec,$this->no_kel);
$data = array(
...
//D:/xampp/htdocs/babakan/app/modules/spp_pbb/controllers/Spp_pbb.php
