...
function upload_data(domisili_wnaid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {domisili_wnaid:domisili_wnaid},
		cache : false,
		dataType : "html",
...
function edit_data(domisili_wnaid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_domisili_wna').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var domisili_wnaid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+domisili_wnaid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(domisili_wnaid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(domisili_wnaid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(domisili_wnaid) {
	$.ajax({
		type : "POST",
