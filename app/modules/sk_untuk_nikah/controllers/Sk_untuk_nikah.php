...
class Sk_untuk_nikah extends Crud_Controller
{
    ...
    {
        $sk_untuk_nikahid = $this->input->post('sk_untuk_nikahid');
        $detail = $this->{$this->modelName}->detail_data($sk_untuk_nikahid);
        // $detail_data = $this->{$this->modelName}->detail_data_detail($sk_untuk_nikahid);
        $kel  = kel_3_dropdown($this->no_prop,$this->no_kab,$this->no_kec,$this->no_kel);
        $negara        = negara_dropdown();
        $status_perkawinan  = status_kawin_2_dropdown();
        $pekerjaan   = pekerjaan_dropdown();
    ...
    public function download()
    {
        ...
        $options['rowModifierCallback'] = function ($row){
            ...
            $ret['alias'] = (trim($ret['alias'])!='' && trim($ret['alias'])!='-') ? ' alias ' . $ret['alias']:''; 
            return $ret;
        };
...
}//D:/xampp/htdocs/babakan/app/modules/sk_untuk_nikah/controllers/Sk_untuk_nikah.php
