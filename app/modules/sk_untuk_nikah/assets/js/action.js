...
function upload_data(sk_untuk_nikahid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_untuk_nikahid:sk_untuk_nikahid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_untuk_nikahid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_untuk_nikah').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_untuk_nikahid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_untuk_nikahid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_untuk_nikahid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_untuk_nikahid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_untuk_nikahid) {
	$.ajax({
		type : "POST",
