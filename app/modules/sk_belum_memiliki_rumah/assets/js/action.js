...
function upload_data(sk_belum_memiliki_rumahid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_belum_memiliki_rumahid:sk_belum_memiliki_rumahid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_belum_memiliki_rumahid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_belum_memiliki_rumah').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_belum_memiliki_rumahid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_belum_memiliki_rumahid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_belum_memiliki_rumahid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_belum_memiliki_rumahid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_belum_memiliki_rumahid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			sk_belum_memiliki_rumahid : sk_belum_memiliki_rumahid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
...
function autocomplete_data_penduduk(identitas){
    ...
    { 
       ...
       extraParams: {
		...
		$("#tanggal_lahir").val(data.tanggal_lahir).datepicker('setDate', format_tanggal_input(data.tgl_lahir));
