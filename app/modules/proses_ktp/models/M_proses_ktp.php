...
class M_proses_ktp extends CI_Model 
{
	...
	{
		if(count($data)>0){
			$this->db->insert('t_proses_ktp',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_proses_ktp',$data);
		}
	}
