...
function upload_data(surat_pengantar_perkawinanid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {surat_pengantar_perkawinanid:surat_pengantar_perkawinanid},
		cache : false,
		dataType : "html",
...
function edit_data(surat_pengantar_perkawinanid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_surat_pengantar_perkawinan').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var surat_pengantar_perkawinanid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+surat_pengantar_perkawinanid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(surat_pengantar_perkawinanid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(surat_pengantar_perkawinanid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(surat_pengantar_perkawinanid) {
	$.ajax({
		type : "POST",
