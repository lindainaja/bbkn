...
class M_surat_pengantar_perkawinan extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('surat_pengantar_perkawinan pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('surat_pengantar_perkawinan',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('surat_pengantar_perkawinan',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('surat_pengantar_perkawinan');
	}
