...
function upload_data(sk_bn_rekeningid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_bn_rekeningid:sk_bn_rekeningid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_bn_rekeningid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_bn_rekening').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_bn_rekeningid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_bn_rekeningid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_bn_rekeningid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_bn_rekeningid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_bn_rekeningid) {
	$.ajax({
		type : "POST",
