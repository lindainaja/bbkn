...
function get_total_data(no_kel,periode,total,key){
	...
	$.ajaxQueue({
		...
		dataType : "json",
		success : function(data) {
			$("#permohonan_"+no_kel).text(data.permohonan);
			$("#perubahan_"+no_kel).text(data.perubahan);
			$("#hilang_"+no_kel).text(data.hilang);

			var total_data = parseFloat(data.permohonan)+parseFloat(data.perubahan)+parseFloat(data.hilang);
