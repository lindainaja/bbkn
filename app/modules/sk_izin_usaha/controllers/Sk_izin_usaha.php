...
class Sk_izin_usaha extends Crud_Controller
{
    ...
    protected function _preAddEditData(&$entry){
        $entry['luas_tempat_usaha'] =str_replace(',','', $entry['luas_tempat_usaha']);
    }
    ...
    {
        $sk_izin_usahaid = $this->input->post('sk_izin_usahaid');
        $detail = $this->m_sk_izin_usaha->detail_data($sk_izin_usahaid);
        // $detail_data = $this->m_sk_izin_usaha->detail_data_detail($sk_izin_usahaid);
        	$kel  = kel_3_dropdown($this->no_prop,$this->no_kab,$this->no_kec,$this->no_kel);
	$agama       = agama_2_dropdown();
	$pekerjaan   = pekerjaan_dropdown();
	...
	'syarat_form' => $this->syarat->form($detail['syarat']),
        	...
        	'sk_izin_usahaid' => $sk_izin_usahaid,
        	'detail' => $detail,
    ...
    public function download()
    {
        ...
        $options['rowModifierCallback'] = function ($row){
            ...
            $ret['nama_instansi'] = config_item('default_nama_instansi');

            $camat = $this->m_sk_izin_usaha->get_camat();
...
}//D:/xampp/htdocs/babakan/app/modules/sk_izin_usaha/controllers/Sk_izin_usaha.php
