...
function upload_data(sk_izin_usahaid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_izin_usahaid:sk_izin_usahaid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_izin_usahaid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_izin_usaha').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_izin_usahaid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_izin_usahaid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_izin_usahaid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_izin_usahaid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_izin_usahaid) {
	$.ajax({
		type : "POST",
