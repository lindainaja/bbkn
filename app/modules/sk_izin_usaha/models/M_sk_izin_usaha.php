...
class M_sk_izin_usaha extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('t_sk_izin_usaha pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('t_sk_izin_usaha',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_sk_izin_usaha',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_sk_izin_usaha');
	}
...
}//D:/xampp/htdocs/babakan/app/modules/sk_izin_usaha/models/M_sk_izin_usaha.php
