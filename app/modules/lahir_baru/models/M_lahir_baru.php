...
class M_lahir_baru extends CI_Model 
{
	...
	{
		if(count($data)>0){
			$this->db->insert('t_lahir_baru',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_lahir_baru',$data);
		}
	}
