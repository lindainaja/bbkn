...
function upload_data(sk_domisili_kantorid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_domisili_kantorid:sk_domisili_kantorid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_domisili_kantorid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_domisili_kantor').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_domisili_kantorid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_domisili_kantorid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_domisili_kantorid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_domisili_kantorid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_domisili_kantorid) {
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/delete_data",
		data : {
			sk_domisili_kantorid : sk_domisili_kantorid
		},
		cache : false,
		dataType : "html",
		beforeSend : function() {
