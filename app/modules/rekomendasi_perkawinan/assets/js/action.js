...
function upload_data(rekomendasi_perkawinanid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {rekomendasi_perkawinanid:rekomendasi_perkawinanid},
		cache : false,
		dataType : "html",
...
function edit_data(rekomendasi_perkawinanid) {
	$.ajax({
		type : "POST",
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_rekomendasi_perkawinan').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var rekomendasi_perkawinanid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+rekomendasi_perkawinanid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(rekomendasi_perkawinanid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(rekomendasi_perkawinanid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(rekomendasi_perkawinanid) {
	$.ajax({
		type : "POST",
