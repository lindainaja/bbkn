...
class M_rekomendasi_perkawinan extends CI_Model 
{
	...
	public function list_data($no_prop=0,$no_kab=0,$no_kec,$no_kel=0,$bln='',$thn='')
	{
		...
		$this->db->from('rekomendasi_perkawinan pk');
	...
	{
		if(count($data)>0){
			$this->db->insert('rekomendasi_perkawinan',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('rekomendasi_perkawinan',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('rekomendasi_perkawinan');
	}
...
}//D:/xampp/htdocs/babakan/app/modules/rekomendasi_perkawinan/models/M_rekomendasi_perkawinan.php
