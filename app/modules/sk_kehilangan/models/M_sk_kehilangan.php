...
class M_sk_kehilangan extends CI_Model 
{
	...
	function detail_data_detail($kehilanganid=0)
	{
		$this->db->select('pkd.*',false);
		$this->db->from('t_kehilangan_detail pkd');
		$this->db->where('pkd.kehilanganid',$kehilanganid);
		$query = $this->db->get();
	...
	{
		if(count($data)>0){
			$this->db->insert('t_kehilangan',$data);
	...
	function update_data($data=array(),$id=0)
	{
		if(count($data)>0){
			$this->db->where('id',$id);
			$this->db->update('t_kehilangan',$data);
		}
	}
	...
	{
		$this->db->where('id',$id);
		$this->db->delete('t_kehilangan');
	}

	function delete_data_detail($kehilanganid=0)
	{
		$this->db->where('kehilanganid',$kehilanganid);
		$this->db->delete('t_kehilangan_detail');
	}
