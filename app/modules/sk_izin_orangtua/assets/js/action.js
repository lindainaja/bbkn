...
function upload_data(sk_izin_orangtuaid){
	$.ajax({
		type : "POST",
		url : base_url + ""+controller+"/form_upload_data",
		data : {sk_izin_orangtuaid:sk_izin_orangtuaid},
		cache : false,
		dataType : "html",
...
function edit_data(sk_izin_orangtuaid) {
	$.ajax({
		type : "POST",
...
function dialog(tpl_html,flag){
	...
	autocomplete_data_penduduk('#ayah_nama');
	autocomplete_data_penduduk('#ibu_nama');
...
function process_add_edit_data(status){
	if(validasi_form()){
		var datas = $('#form_sk_izin_orangtua').serializeArray();
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				...
				if(status=='simpan_download'){
					var sk_izin_orangtuaid = data.id
					window.location = base_url + 'index.php/'+controller+'/download/id/'+sk_izin_orangtuaid;
				}
				load_data();
			},
			error : function(xhr, ajaxOptions, thrownError) {
...
function hapus_data(sk_izin_orangtuaid){
	jQuery.alerts.dialogClass = 'alert-warning';
	jConfirm('Apakah anda yakin ingin menghapus data ini?', 'Konfirmasi?', function (r) {
		if (r == true) {
			processDeleteData(sk_izin_orangtuaid);//PROSES PENGHAPUSAN DATA
		}//end if true
	});
}

function processDeleteData(sk_izin_orangtuaid) {
	$.ajax({
		type : "POST",
